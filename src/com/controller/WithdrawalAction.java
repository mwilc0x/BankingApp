...
public class WithdrawalAction extends LookupDispatchAction {
	
	@Override
	protected Map getKeyMethodMap() {
		Map<String,String> map = new HashMap<String, String>();
		map.put("WithdrawalForm.Withdrawal", "Withdrawal");
		
		return map;
	}
	
	public ActionForward Withdrawal(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		WithdrawalForm df = (WithdrawalForm) form;
        ...
        Criteria criteria = session.createCriteria(WithdrawalForm.class);
        criteria.add(Restrictions.eq("cust_id", userList.get(0).getPk_id()));
        criteria.add(Restrictions.eq("type", df.getType()));
        
        List<WithdrawalForm> users = (List<WithdrawalForm>)criteria.list();
        ...
        trans.setTrans_type("withdrawal");
        
        if(users.size() > 0) {
        	// has account, update
            WithdrawalForm user = users.get(0);
            System.out.println("Use	r "+user.getName()+" currently has a balance of " + user.getBalance());
